...
void send_joystick_value(){
    ...
    output[8]  = ' ';
    // x-dir
    output[9]  = joy_val_32_x;       /* 0xef */
    output[10] = joy_val_32_x >> 8;  /* 0xbe */
    output[11] = joy_val_32_x >> 16; /* 0xad */
    output[12] = joy_val_32_x >> 24; /* 0xde */ 
    //y-dir
    output[13] = joy_val_32_y;       /* 0xef */
    output[14] = joy_val_32_y >> 8;  /* 0xbe */
    output[15] = joy_val_32_y >> 16; /* 0xad */
    output[16] = joy_val_32_y >> 24; /* 0xde */ 
    // Button
    output[17] = (uint8_t) joy_button_val;
    output[18] = '\r';
    output[19] = '\n';

    while (ble_nus_c_string_send(&m_ble_nus_c, output, length) != NRF_SUCCESS)
    {
        // repeat until sent.
    } 
...
void send_joystick_value_handle(app_uart_evt_t * p_event){
        send_joystick_value();
        nrf_delay_ms(100);
}
